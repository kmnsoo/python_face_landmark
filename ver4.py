    ...
    while True:
        try:
            ...
            #입력된 숫자에 따라 index 변수에 앞에서 지정한 리스트를 대입한다.
                ...
                # show_cv2('result_', 2)
                th = Process(target = show_cv2, args = ('result_', 2, ))
                th.start()
                th.join()
            elif key == ord('4'):
                index = NOSE
...
if __name__ == "__main__":
    # show_cv2('img', 1)
    th1 = Process(target = show_cv2, args = ('img', 1, ))
    th1.start()
    th1.join()
    #객체 해제
    # cap.release()
